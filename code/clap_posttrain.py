...
def load_embedding(path):
    ...
    if emb.shape[-1] != 512:
        raise RuntimeError(f"expected embedding dim=512, got {tuple(emb.shape)} in {path}")
...
class EmbeddingCSVDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = load_embedding(pt_path)  # (1,512)
        y = torch.tensor(y, dtype=torch.float32)
        return x, y

def collate_fn(batch):
    xs, ys = zip(*batch)
    xs = torch.stack(xs, dim=0)  # (B,1,512)
...
def main():
    ...
    for epoch in range(EPOCHS):
        ...
        for x, y in pbar:
            x = x.to(device, non_blocking=True)   # (B,1,512)
